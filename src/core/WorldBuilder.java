...
public class WorldBuilder {
    ...
    private static GameEvent instantiateEvent(String eventName) {
        if (eventName == null || eventName.isEmpty()) return null;
        try {
            String fullPath = "events." + eventName;

            Class<?> clazz = Class.forName(fullPath);
            return (GameEvent) clazz.getDeclaredConstructor().newInstance();
        } catch (ClassNotFoundException e) {
            System.err.println("Ошибка: Класс " + eventName + " не найден в папке events.");
            return null;
        } catch (Exception e) {
            System.err.println("Не удалось создать событие: " + eventName);
            e.printStackTrace();
            return null;
        }
    }
        ...
        try (FileReader reader = new FileReader(filePath)) {
            ...
            for (LocationData data : dataList) {
                ...
                if (data.eventName != null) {
                    GameEvent event = instantiateEvent(data.eventName);
                    loc.setEvent(event);
                }
